...
A(4,seg1)  =  [T(2)^7, T(2)^6, T(2)^5, T(2)^4, T(2)^3, T(2)^2, T(2), 1];
bx(4)      =  X(2);
by(4)      =  Y(2);
% Initial position for second
A(5,seg2)  =  [T(2)^7, T(2)^6, T(2)^5, T(2)^4, T(2)^3, T(2)^2, T(2), 1];
bx(5)      =  X(2);
by(5)      =  Y(2);
% Velocities must be the same for both segments
A(6,seg1)  =  [7*T(2)^6, 6*T(2)^5, 5*T(2)^4 4*T(2)^3 3*T(2)^2 2*T(2) 1 0];
A(6,seg2)  = -[7*T(2)^6, 6*T(2)^5, 5*T(2)^4 4*T(2)^3 3*T(2)^2 2*T(2) 1 0];
bx(6)      =  0;
by(6)      =  0;
% Accelerations must be the same for both segments
A(7,seg1)  =  [42*T(2)^5, 30*T(2)^4, 20*T(2)^3 12*T(2)^2 6*T(2) 2 0 0];
A(7,seg2)  = -[42*T(2)^5, 30*T(2)^4, 20*T(2)^3 12*T(2)^2 6*T(2) 2 0 0];
bx(7)      =  0;
by(7)      =  0;
% Jerk must be the same for both segments
A(8,seg1)  =  [210*T(2)^4, 120*T(2)^3, 60*T(2)^2 24*T(2) 6 0 0 0];
A(8,seg2)  = -[210*T(2)^4, 120*T(2)^3, 60*T(2)^2 24*T(2) 6 0 0 0];
bx(8)      =  0;
by(8)      =  0;
% Snap must be the same for both segments
A(9,seg1)  =  [630*T(2)^3, 360*T(2)^2, 120*T(2) 24 0 0 0 0];
A(9,seg2)  = -[630*T(2)^3, 360*T(2)^2, 120*T(2) 24 0 0 0 0];
bx(9)      =  0;
by(9)      =  0;
...
Cx = A \ bx;
Cy = A \ by;
fprintf('Coefficients for x, segment 1: (%0.4f)*t^5 + (%0.4f)*t^4 + (%0.4f)*t^3 + (%0.4f)*t^2 + (%0.4f)*t + (%0.4f)\n',Cx(1),Cx(2),Cx(3),Cx(4),Cx(5),Cx(6));
fprintf('Coefficients for y, segment 1: (%0.4f)*t^5 + (%0.4f)*t^4 + (%0.4f)*t^3 + (%0.4f)*t^2 + (%0.4f)*t + (%0.4f)\n',Cy(1),Cy(2),Cy(3),Cy(4),Cy(5),Cy(6));
fprintf('Coefficients for x, segment 2: (%0.4f)*t^5 + (%0.4f)*t^4 + (%0.4f)*t^3 + (%0.4f)*t^2 + (%0.4f)*t + (%0.4f)\n',Cx(7),Cx(8),Cx(9),Cx(10),Cx(11),Cx(12));
fprintf('Coefficients for y, segment 2: (%0.4f)*t^5 + (%0.4f)*t^4 + (%0.4f)*t^3 + (%0.4f)*t^2 + (%0.4f)*t + (%0.4f)\n',Cy(7),Cy(8),Cy(9),Cy(10),Cy(11),Cy(12));
...
scatter(X,Y)
t1 = (T(1):0.1:T(2))';
xt1 = [ t1.^7 t1.^6 t1.^5 t1.^4 t1.^3 t1.^2 t1.^1 ones(size(t1)) ]*Cx(1:6);
yt1 = [ t1.^7 t1.^6 t1.^5 t1.^4 t1.^3 t1.^2 t1.^1 ones(size(t1)) ]*Cy(1:6);
t2 = (T(2):0.1:T(3))';
xt2 = [ t2.^7 t2.^6 t2.^5 t2.^4 t2.^3 t2.^2 t2.^1 ones(size(t2)) ]*Cx(7:12);
yt2 = [ t2.^7 t2.^6 t2.^5 t2.^4 t2.^3 t2.^2 t2.^1 ones(size(t2)) ]*Cy(7:12);
hold on;
plot(xt1,yt1)
plot(xt2,yt2)
